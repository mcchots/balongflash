...
printf("\n Selecting sections from the firmware file:\n\n ## Offset Size Name\n-------------------------------------");
for (i=0;i<npart;i++) {  
...
if (res != 6) {
  printf("\n Incorrect answer length for ^DATAMODE");
  exit(-2);
}  
if (memcmp(replybuf,OKrsp,6) != 0) {
  printf("\n ^DATAMODE command rejected, digital signature may be required\n");
  exit(-2);
}  
}
...
printf("\n Title: version %i, match code: %8.8s",ptable[0].hd.hdversion,ptable[0].hd.unlock);
}
