...
if (masslen != 0) {
 ...
 if (res != (masslen-1)) {
   printf("\nModem response too short: %i bytes, %i bytes expected\n",res+1,masslen);
...
if (strspn(dentry->d_name+3,"0123456789AaBbCcDdEeFf") != 8) {
  printf("\n Section identifier error - non-numeric character - %s\n",filename);
  exit(1);
}  
